...
class AsyncWikipediaAPI:
    """An async class to interact with Wikipedia's MediaWiki API."""
    
    def __init__(self, language: str = "en"):
        """
        Initialize the asynchronous Wikipedia API client.
    ...
    async def search(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        Asynchronously search for Wikipedia articles.
    ...
    async def get_page_summary(self, title: str) -> Optional[Dict[str, Any]]:
        """
        Asynchronously get a summary of a Wikipedia page.
    ...
    async def get_page_content(self, title: str) -> Optional[str]:
        """
        Asynchronously get the full content of a Wikipedia page.
    ...
    async def get_page_extract(self, title: str, sentences: int = 3) -> Optional[str]:
        """
        Asynchronously get a plain text extract of a Wikipedia page.
    ...
    async def get_page_categories(self, title: str) -> List[str]:
        """
        Asynchronously get categories for a Wikipedia page.
    ...
    async def get_page_links(self, title: str, limit: int = 100) -> List[str]:
        """
        Asynchronously get links from a Wikipedia page.
    ...
    async def get_page_images(self, title: str) -> List[Dict[str, str]]:
        """
        Asynchronously get images from a Wikipedia page.
    ...
    async def get_random_pages(self, count: int = 1) -> List[str]:
        """
        Asynchronously get random Wikipedia page titles.
    ...
    async def page_exists(self, title: str) -> bool:
        """
        Asynchronously check if a Wikipedia page exists.
    ...
    async def get_page_info(self, title: str) -> Optional[Dict[str, Any]]:
        """
        Asynchronously get basic information about a Wikipedia page.
...
async def async_search_wikipedia(query: str, language: str = "en", limit: int = 10) -> List[Dict[str, Any]]:
    """Quick async search function."""
    api = AsyncWikipediaAPI(language)
    return await api.search(query, limit)

async def async_get_wikipedia_summary(title: str, language: str = "en") -> Optional[Dict[str, Any]]:
    """Quick async summary function."""
    api = AsyncWikipediaAPI(language)
    return await api.get_page_summary(title)

async def async_get_wikipedia_extract(title: str, language: str = "en", sentences: int = 3) -> Optional[str]:
    """Quick async extract function."""
    api = AsyncWikipediaAPI(language)
    return await api.get_page_extract(title, sentences)
...
if __name__ == "__main__":
    ...
    async def async_example():
        ...
        # Using async API
        async_api = AsyncWikipediaAPI()
        
        # Search for articles asynchronously
        results = await async_api.search("Machine learning", limit=3)
        print("Async search results:")
        for result in results:
            print(f"- {result['title']}")
        
        # Get multiple page summaries concurrently
        import asyncio
        titles = ["Artificial intelligence", "Machine learning", "Deep learning"]
        summaries = await asyncio.gather(*[
            async_api.get_page_summary(title) for title in titles
        ])
        
        print("\nConcurrent summaries:")
        for i, summary in enumerate(summaries):
            if summary:
                print(f"{titles[i]}: {summary.get('extract', '')[:100]}...")
